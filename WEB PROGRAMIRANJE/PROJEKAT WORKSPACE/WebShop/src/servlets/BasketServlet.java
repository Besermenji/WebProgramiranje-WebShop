...
public class BasketServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//brisanje iz korpe
		if(type.equals("delete_product")){
			Reciept rec = db.getBasket(user);
			if(identifier.equals("namestaj")){
				rec.removeProduct(name);
				;
				
			}
			else if(identifier.equals("usluga")){
				rec.removeService(name, price);
			}
			db.createBasket(rec, user);
			response.getWriter().write("true "+price);
			return;
		}
