		...
		$.ajax({
	        ...
	        success: function (data) {
	        	for(var i = 0; i < data.length;i++){
	        		...
	        		//punjenje tabel
	        		$("#namestaj_bodi").append("<tr id='"+codeID+"'>"+code+slika+naziv+boja+zemlja+proizvodjac+actiono+starto+endo+percento+cenos+kolikos+besplatnos+categoros+yearos+salonos+videos+"</tr>");
	        	}
		...
		$.ajax({
			...
			success: function(data){
				for(var i = 0; i < data.length; i++){
					...
					$("#usluge_bodi").append("<tr>"+ime+opis+cena+"</tr>");
